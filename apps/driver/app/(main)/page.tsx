...
export default function HomePage() {
  ...
  return (
    <div className={!isOnline && !transitioning ? 'min-h-screen bg-background pb-28' : 'absolute inset-0 bg-background overflow-hidden'}>

      {/* ═══ ONLINE / transitioning: full-bleed map + floating order flow ═══ */}
      {(isOnline || transitioning) && (
      <>
      ...
      {transitioning && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-background/95 backdrop-blur-sm">
          ...
          <p className="text-on-surface font-black text-xl mt-8 tracking-tight">
      ...
      <div className="absolute top-0 left-0 right-0 z-30" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
          ...
          >
            {isOnline ? (
              <>
                ...
                <span className="text-on-surface font-black text-sm">Online</span>
                <span className="text-neighborhood-green/70 text-xs font-semibold">· tap to go off</span>
              </>
            ) : (
              <>
                <span className="text-on-surface-variant text-xs">⚫</span>
                <span className="text-on-surface font-black text-sm">Offline</span>
                <span className="text-on-surface-variant text-xs font-semibold">· tap GO</span>
              </>
            )}
          </button>
        ...
        {!loading && !loadError && data?.profile?.kyc_status && data.profile.kyc_status !== 'approved' && (
          <Link
            href="/onboarding"
            className="mx-3 mt-2 flex items-center gap-2 bg-primary-fixed border border-primary-container/30 rounded-2xl px-3 py-2.5 backdrop-blur shadow-xl"
          >
            <AlertTriangle size={15} className="text-primary flex-shrink-0" />
            <span className="flex-1 text-on-primary-fixed font-bold text-xs leading-tight">
            ...
            <ChevronRight size={14} className="text-primary/70 flex-shrink-0" />
          </Link>
        )}
        ...
        {loadError && (
          ...
          >
            ...
            <span className="text-error font-bold text-xs">Failed to load — tap to retry</span>
          </button>
        )}
      </div>
        ...
        >
          ...
          {data?.activeOrder ? (
            ...
            >
              <span
                ...
                style={{ background: 'radial-gradient(circle, rgba(242,101,34,0.18), transparent 70%)' }}
              />
              <div className="relative flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="relative w-2 h-2 flex-shrink-0">
                      <span className="absolute inset-0 rounded-full bg-primary-container animate-ping opacity-50" />
                      <span className="absolute inset-0 rounded-full bg-primary-container" />
                    </span>
                    <p className="text-[10px] font-black text-primary uppercase tracking-widest">Active Delivery</p>
                  </div>
                  <p className="font-black text-on-surface text-base truncate">{data.activeOrder.food_maker?.display_name ?? 'Order'}</p>
          ...
          {stackCandidates.length > 0 && !acceptedOrderId && (
            <div className="space-y-2">
              <p className="text-xs font-black text-primary uppercase tracking-wider px-1">
              ...
              {stackCandidates.slice(0, 2).map((candidate) => {
                ...
                return (
                  <div
                    ...
                    className="bg-primary-fixed/90 border border-primary-container/30 rounded-2xl overflow-hidden backdrop-blur-sm"
                  >
                    <div className="flex items-start justify-between px-4 pt-4 pb-3">
                      <div className="flex-1 min-w-0 pr-3">
                        <p className="font-black text-on-surface text-sm leading-tight truncate">
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        {candidate.tip_amount > 0 && (
                          <p className="text-neighborhood-green text-xs font-black mt-1">
          ...
          {acceptedOrderId && (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 px-4 py-3.5 bg-neighborhood-green/12 border border-green-500/25 rounded-2xl backdrop-blur-sm">
                <CheckCircle size={22} className="text-neighborhood-green flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-black text-neighborhood-green">Order Accepted!</p>
                  <p className="text-xs text-neighborhood-green/60 mt-0.5">
              ...
              {stackCandidates.length > 0 ? (
                        ...
                        <div className="flex items-start justify-between px-4 pt-4 pb-3">
                          <div className="flex-1 min-w-0 pr-3">
                            ...
                            <div className="flex items-center gap-2 mt-1.5">
                              ...
                              <span className="text-on-surface-variant text-[10px]">•</span>
                              <span className="text-on-surface-variant text-xs">{Math.round(candidate.score)}% match</span>
                            </div>
                          </div>
                          <div className="text-right flex-shrink-0">
                            <p className="font-black text-xl leading-none text-primary">
                ...
                <p className="text-center text-outline text-xs py-2">No compatible add-on orders nearby right now</p>
              )}
            </div>
          )}
          ...
          {!acceptedOrderId && orders.length > 0 ? (
            <div className="space-y-3">
              {orders.map(order => {
                ...
                return (
                  <div
                    key={order.id}
                    className="bg-surface-container-lowest/95 border border-outline-variant/30 rounded-2xl overflow-hidden backdrop-blur-sm"
                  >
                    {/* ── Card header: restaurant name + earnings ── */}
                    <div className="flex items-start justify-between px-4 pt-4 pb-3 border-b border-outline-variant/30">
                      <div className="flex-1 min-w-0 pr-3">
                        <p className="font-black text-on-surface text-base leading-tight truncate">
                          {order.food_maker?.display_name ?? 'Kitchen'}
                        </p>
                        <div className="flex items-center gap-2 mt-1.5">
                          {order.status === 'preparing' ? (
                            <span className="flex items-center gap-1 bg-primary-fixed border border-primary-container/30 rounded-full px-2 py-0.5 text-[10px] font-black text-primary uppercase tracking-wide">
                              <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse inline-block" />
                              Preparing
                            </span>
                          ) : (
                            <span className="flex items-center gap-1 bg-neighborhood-green/15 border border-green-500/30 rounded-full px-2 py-0.5 text-[10px] font-black text-neighborhood-green uppercase tracking-wide">
                              <span className="w-1.5 h-1.5 rounded-full bg-green-400 inline-block" />
                              Ready
                            </span>
                          )}
                          <Clock size={11} className="text-on-surface-variant flex-shrink-0" />
                          <span className="text-on-surface-variant text-xs font-semibold">{timeAgo(order.created_at)}</span>
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-black text-2xl leading-none" style={{ color: '#a63b00' }}>
                    ...
                    {/* ── Mini route map ── */}
                    {hasMap && (
                      <div className="relative w-full h-36 border-b border-outline-variant/30">
                        <RoutePreviewMap
                          pickupLat={makerLat}
                          pickupLng={makerLng}
                          dropoffLat={dropLat}
                    ...
                    <div className="px-4 py-3 space-y-2">
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <span className="w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1" style={{ backgroundColor: '#f26522' }} />
                        <div className="flex-1 min-w-0">
                          <span className="text-on-surface text-sm font-semibold truncate block">
                            {order.food_maker?.display_name ?? 'Kitchen'}
                          </span>
                          {pickupAddrStr && (
                            <span className="text-on-surface-variant text-xs truncate block mt-0.5">
                              {pickupAddrStr}
                            </span>
                          )}
                        </div>
                        {toPickupM != null && (
                          <div className="flex flex-col items-end gap-0.5 flex-shrink-0">
                            ...
                            <span className="bg-primary-fixed text-primary text-[10px] font-black px-1.5 py-0.5 rounded-full">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        {!hasMap && deliveryM != null && (
                          <span className="text-outline text-[10px] font-semibold">{formatDistance(deliveryM)} run</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <span className="w-2.5 h-2.5 rounded-full border-2 border-tertiary flex-shrink-0 mt-1" />
                        <div className="flex-1 min-w-0">
                          <span className="text-on-surface text-sm font-semibold truncate block">
                        ...
                        {deliveryM != null && (
                          <span className="bg-tertiary/10 text-tertiary text-[10px] font-black px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5">
                            {formatDistance(deliveryM)}
                          </span>
                        )}
                      </div>
                    ...
                    {acceptError?.orderId === order.id && (
                      <div className="mx-4 mb-2 px-3 py-2 bg-red-500/15 border border-red-500/25 rounded-xl text-xs text-error font-semibold">
                        {acceptError.message}
                      </div>
                    )}
                    ...
                    <div className="px-4 pb-4">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 rounded-xl py-3.5 font-black text-sm tracking-wide text-on-surface active:scale-[0.98] transition-all duration-100 disabled:opacity-50"
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-surface-container-lowest/80 border border-outline-variant/30 rounded-2xl">
              <span className="text-lg leading-none">🛵</span>
              <span className="text-on-surface-variant text-sm font-semibold">Waiting for orders…</span>
            </div>
          ) : null}
      ...
      {/* ═══ OFFLINE: Neighborly Modern light "go online" home ═══ */}
      {!isOnline && !transitioning && (
      <>
        {/* Top app bar */}
        <header className="sticky top-0 z-30 bg-surface flex items-center justify-between px-5 h-16">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSideMenuOpen(true)}
              aria-label="Open menu"
              className="w-10 h-10 -ml-2 rounded-full flex items-center justify-center text-primary active:bg-surface-variant transition-colors"
            >
              <Menu size={22} />
            </button>
          ...
          <Link
            ...
            className="relative w-10 h-10 rounded-full flex items-center justify-center text-primary active:bg-surface-variant transition-colors"
          >
            <span className="text-base" aria-hidden>🔔</span>
            {unreadCount > 0 && (
        ...
        <main className="px-5 space-y-5 pt-2">
          {/* Today's earnings card (real data — no invented goal) */}
          <section className="bg-surface-container-lowest rounded-xl p-6 border border-outline-variant/30">
            <div className="flex items-center justify-between mb-1">
              <p className="font-mono text-xs text-on-surface-variant uppercase tracking-widest">Today so far</p>
              {(data?.todayDeliveries ?? 0) > 0 && (
                <span className="font-mono text-[11px] tracking-wide px-3 py-1 rounded-full bg-neighborhood-green/10 text-neighborhood-green">
                  ON THE MOVE
                </span>
              )}
            </div>
            <p className="font-display text-5xl font-extrabold text-primary tracking-tight leading-none mt-1">
              ${(data?.todayEarnings ?? 0).toFixed(2)}
            </p>
            <p className="text-sm text-on-surface-variant mt-2">
              {data?.todayDeliveries ?? 0} {(data?.todayDeliveries ?? 0) === 1 ? 'delivery' : 'deliveries'} today
              {(data?.weekEarnings ?? 0) > 0 && (
                <span className="text-outline"> · ${(data?.weekEarnings ?? 0).toFixed(2)} this week</span>
              )}
            </p>
          </section>

          {/* KYC banner (offline) */}
          {data?.profile?.kyc_status && data.profile.kyc_status !== 'approved' && (
            <Link
              href="/onboarding"
              className="flex items-center gap-2 bg-primary-fixed border border-primary-container/30 rounded-xl px-4 py-3"
            >
              <AlertTriangle size={16} className="text-primary flex-shrink-0" />
              <span className="flex-1 text-on-primary-fixed font-semibold text-sm leading-tight">
                {data.profile.kyc_status === 'pending_review' ? "Verification under review — we'll notify you when approved"
                  : data.profile.kyc_status === 'rejected' ? 'Verification rejected — tap to resubmit'
                  : 'Identity verification required to accept orders'}
              </span>
          ...
          {/* Map card */}
          <section className="relative rounded-xl overflow-hidden border border-outline-variant/30 h-64">
            <LiveMap lat={lat} lng={lng} isOnline={isOnline} />
          </section>

          {/* Go online prompt */}
          <div className="text-center pt-1">
            <p className="font-display text-xl font-bold text-on-surface">Ready to support your neighbors?</p>
          </div>

          <button
            onClick={toggleOnline}
            disabled={toggling || transitioning}
            className="w-full flex items-center justify-center gap-3 bg-primary text-on-primary rounded-full py-4 active:scale-[0.98] transition-transform disabled:opacity-60"
          >
            <span className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center">
              <Truck size={18} />
            </span>
            <span className="font-display text-lg font-bold tracking-wide">GO ONLINE</span>
          </button>

          {/* Real stat tiles */}
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-surface-container rounded-xl p-4 text-center">
              <p className="font-mono text-xs text-on-surface-variant uppercase tracking-wide mb-1">Rating</p>
              <p className="font-mono text-xl text-primary">{data?.profile?.avg_rating?.toFixed(1) ?? '—'}</p>
            </div>
            <div className="bg-surface-container rounded-xl p-4 text-center">
              <p className="font-mono text-xs text-on-surface-variant uppercase tracking-wide mb-1">Deliveries</p>
              <p className="font-mono text-xl text-neighborhood-green">{data?.profile?.total_deliveries ?? 0}</p>
            </div>
          </div>
          ...
          {loadError && (
            <button
              onClick={load}
              className="w-full flex items-center justify-center gap-2 bg-error-container border border-error/30 rounded-xl px-4 py-3"
            >
              <AlertTriangle size={15} className="text-error" />
              <span className="text-on-error-container font-semibold text-sm">Failed to load — tap to retry</span>
            </button>
          )}
        </main>
      </>
      ...
      >
        ...
        {/* Drawer */}
        <div
          className={`absolute top-0 bottom-0 left-0 w-[280px] bg-surface border-r border-outline-variant/40 shadow-2xl transition-transform duration-300 ease-out flex flex-col ${
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-outline-variant/30">
            <p className="text-on-surface font-black text-xl tracking-tight">Doornext</p>
            <button
              onClick={closeSideMenu}
              aria-label="Close menu"
              className="w-9 h-9 rounded-xl bg-surface-container-high border border-outline-variant/30 flex items-center justify-center active:scale-95 transition-transform"
            >
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-on-surface font-bold text-sm truncate">{data?.profile?.full_name ?? 'Nexter'}</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                <Star size={11} className="text-primary fill-amber-400" />
                <span className="text-on-surface-variant text-xs font-semibold">
                  {data?.profile?.avg_rating != null ? data.profile.avg_rating.toFixed(1) : '—'}
                </span>
                <span className="text-outline text-xs">·</span>
                <span className="text-on-surface-variant text-xs">{data?.profile?.total_deliveries ?? 0} trips</span>
              </div>
            </div>
          ...
          {!loading && (
            <div className="px-4 py-4 border-b border-outline-variant/30 space-y-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Today&apos;s earnings</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <p className="font-black text-on-surface text-3xl leading-none tracking-tight">
                    ${(data?.todayEarnings ?? 0).toFixed(2)}
                  </p>
                ...
                <p className="text-xs text-outline mt-1.5">
                  <span className="text-on-surface-variant font-semibold">${(data?.weekEarnings ?? 0).toFixed(0)}</span> this week
                </p>
              </div>
              <div className="grid grid-cols-2 gap-1.5">
                <div className="bg-surface-container-high border border-outline-variant/30 rounded-xl py-2 px-3">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-outline">Accept</p>
                  <p className={`text-base font-black leading-none mt-0.5 ${
                    data?.profile?.acceptance_rate == null ? 'text-on-surface-variant'
                    : data.profile.acceptance_rate >= 80 ? 'text-neighborhood-green'
                    : data.profile.acceptance_rate >= 60 ? 'text-primary'
                    : 'text-error'
                  }`}>
                ...
                <div className="bg-surface-container-high border border-outline-variant/30 rounded-xl py-2 px-3">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-outline">On-Time</p>
                  <p className={`text-base font-black leading-none mt-0.5 ${
                    data?.profile?.on_time_delivery_rate == null ? 'text-on-surface-variant'
                    : data.profile.on_time_delivery_rate >= 85 ? 'text-neighborhood-green'
                    : data.profile.on_time_delivery_rate >= 65 ? 'text-primary'
                    : 'text-error'
                  }`}>
                ...
                <div className="bg-surface-container-high border border-outline-variant/30 rounded-xl py-2 px-3">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-outline">Complete</p>
                  <p className={`text-base font-black leading-none mt-0.5 ${
                    data?.profile?.completion_rate == null ? 'text-on-surface-variant'
                    : data.profile.completion_rate >= 90 ? 'text-neighborhood-green'
                    : data.profile.completion_rate >= 70 ? 'text-primary'
                    : 'text-error'
                  }`}>
                ...
                <div className="bg-surface-container-high border border-outline-variant/30 rounded-xl py-2 px-3">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-outline">Issues</p>
                  <p className={`text-base font-black leading-none mt-0.5 ${
                    (data?.profile?.issues_reported ?? 0) === 0 ? 'text-neighborhood-green'
                    : (data?.profile?.issues_reported ?? 0) <= 3 ? 'text-primary'
                    : 'text-error'
                  }`}>
          ...
          <nav className="flex-1 py-2 overflow-y-auto">
            {[
              ...
              <Link
                ...
                href={item.href}
                onClick={closeSideMenu}
                className="flex items-center gap-3 px-4 py-3 active:bg-surface-container-low transition-colors"
              >
          ...
          <div className="px-4 py-3 border-t border-outline-variant/30">
            <p className="text-[10px] text-outline font-semibold tracking-wide">Nexter v1.0.0</p>
          </div>
        </div>
      </div>
