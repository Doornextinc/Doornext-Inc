...
try {
  ...
  messaging.onBackgroundMessage((payload) => {
    ...
    self.registration.showNotification(title, {
      ...
      data,
      tag: data.order_id ?? 'doornext-driver',
  ...
  self.addEventListener('notificationclick', (event) => {
    ...
    // new_order alerts → home (/) so driver can accept; active order updates → /active
    const url = data.type === 'new_order' ? '/' : (data.order_id ? '/active' : '/')
    event.waitUntil(
