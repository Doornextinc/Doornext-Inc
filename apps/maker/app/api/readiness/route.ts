 ...
 * GET /api/readiness
 *
 * Deep dependency probe for the maker app.
...
export async function GET(req: NextRequest) {
  ...
  if (!supabaseUrl || !serviceKey) {
    ...
    try {
      ...
      const { error } = await sb.from('food_makers').select('id').limit(1)
  ...
  // ── Storage (maker documents bucket) ─────────────────────────────────────
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    ...
    try {
      ...
      const { error } = await sb.storage.getBucket('maker-documents')
      if (error) throw new Error(error.message)
      checks.storage = { status: 'ok' }
    } catch (err) {
      checks.storage = { status: 'error', detail: String(err) }
    }
  }
